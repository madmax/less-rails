  ...
  module Rails
    class LessTemplate < Tilt::LessTemplate
      ...
      def config_to_less_parser_options(scope)
        ...
        {:filename => eval_file, :line => line, :paths => paths, dumpLineNumbers: config_dump_line_numbers(scope)}
      end
      ...
      def config_dump_line_numbers(scope)
        config_from_rails(scope)[:dump_line_numbers]
      end
